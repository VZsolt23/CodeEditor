...
**Phases 1–5 are complete, as is the Ember design overhaul.** The editor is a working daily driver with full C# language intelligence (Roslyn: live diagnostics with squiggles, completion, Quick Info, signature help, Go To Definition / Find References / Rename, Format Document, semantic highlighting) and full TypeScript/JavaScript/HTML/CSS intelligence over LSP (diagnostics, hover, completion, definition, references, rename, format, plus HTML/XML auto-closing tags) — on top of tabbed editing, a workspace explorer with Devicon file icons, Find in Files, in-editor find/replace, Output/Problems/Terminal panels, and session restore — all wearing the unique warm-graphite "Ember" identity (see [DESIGN.md](DESIGN.md)). Phase 6 (framework support, JSON/XML validation and folding, performance) is next.
...
### Done — Phase 5: LSP (TypeScript / JavaScript / HTML / CSS)
...
- [x] HTML/XML auto-closing tags: typing `>` on an opening tag inserts the matching closing tag with the caret between them. Pure parsing in `Core.Documents.TagAutoCloser` (skips closing/self-closing tags, comments, declarations, HTML void elements, invalid names; XML has no void elements); `EditorView` does the AvalonEdit backward scan for the `<`, bounded, and won't duplicate an existing close tag. Enabled for `html` and `xml` language ids
...
- [ ] Refactor: a common `ILanguageService` facade routed by `LanguageInfo.Id`, to collapse the C#-vs-LSP branching now repeated across several `DocumentViewModel`/`MainViewModel` methods
