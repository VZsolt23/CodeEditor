...
- [ ] HTML/CSS language servers (`vscode-html/css-languageservice`-based servers) reusing the same client. (All TS/JS features — diagnostics, hover, completion, definition, references, rename, format — are done; a common `ILanguageService` facade routed by `LanguageInfo.Id` is worth considering before adding more backends)
