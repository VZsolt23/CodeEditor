...
- [x] TS/JS go to definition (`F12`): `LspServerConnection.RequestDefinitionAsync` (`textDocument/definition`, handles Location / Location[] / LocationLink[]; reads target-file lines for excerpts) → `ILspService.GetDefinitionsAsync` → `DocumentViewModel.GetDefinitionsAsync` routes by language (flush + 0-based position for LSP). Reuses the Roslyn result plumbing (single result navigates via `PendingNavigation`; multiple go to the search-results panel). Gating split: new `CanGoToDefinition` enables F12 for C# and LSP languages, while references/rename/format stay C#-only (`CanUseLanguageServices`) until their LSP paths land
...
- [ ] Map remaining LSP features: references, rename, formatting → same editor plumbing Roslyn uses (hover + completion + definition done; consider a common `ILanguageService` facade routed by `LanguageInfo.Id`)
